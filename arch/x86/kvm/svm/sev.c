...
static int __sev_snp_update_protected_guest_state(struct kvm_vcpu *vcpu)
{
	...
	if (VALID_PAGE(svm->sev_es.snp_vmsa_pfn)) {
		/*
...
void sev_snp_init_protected_guest_state(struct kvm_vcpu *vcpu)
{
	...
	if (!svm->sev_es.snp_vmsa_update_on_init)
		goto unlock;

	svm->sev_es.snp_vmsa_update_on_init = false;
...
static int sev_snp_ap_creation(struct vcpu_svm *svm)
{
	...
	target_svm->sev_es.snp_vmsa_update_on_init = false;
	...
	case SVM_VMGEXIT_AP_CREATE_ON_INIT:
		kick = false;
		target_svm->sev_es.snp_vmsa_update_on_init = true;
		fallthrough;
