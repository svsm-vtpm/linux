...
static void svm_set_efer(struct kvm_vcpu *vcpu, u64 efer)
{
	...
	svm_efer_write(to_svm(vcpu), efer | EFER_SVME);
...
static void svm_free_vcpu(struct kvm_vcpu *vcpu)
{
	...
	if (sev_es_guest(vcpu->kvm)) {
		 ...
		 * state, be sure to flush it before returning it to the
		 * system. This is done using the VM Page Flush MSR (which
		 * takes the page virtual address and guest ASID).
		 */
...
{
	return svm_dr6_read(to_svm(vcpu));
}
...
static int vmload_interception(struct vcpu_svm *svm)
{
	...
	ret = kvm_vcpu_map(&svm->vcpu,
			   gpa_to_gfn(svm_rax_read(svm)), &map);
	if (ret) {
		if (ret == -EINVAL)
			kvm_inject_gp(&svm->vcpu, 0);
		return 1;
