...
static int __init setup_kvm_sev_migration(void)
{
	...
	if (!efi_enabled(EFI_RUNTIME_SERVICES)) {
		pr_info("setup_kvm_sev_migration: EFI runtime services are not enabledi\n");
		return 0;
	}
	...
	if (status == EFI_NOT_FOUND) {
		pr_info("setup_kvm_sev_migration: EFI live migration variable not found\n");
		return 0;
	}

	if (status != EFI_SUCCESS) {
		pr_info("setup_kvm_sev_migration: EFI variable retrieval failed\n");
		return 0;
	}

	if (enabled == 0) {
		pr_info("setup_kvm_sev_migration: live migration disabled in EFI\n");
		return 0;
	}

	pr_info("setup_kvm_sev_migration: live migration enabled in EFI\n");
