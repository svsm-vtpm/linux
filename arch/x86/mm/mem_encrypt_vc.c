...
static u64 vmg_rdtsc(struct ghcb *ghcb, unsigned long ghcb_pa,
		     struct pt_regs *regs, struct insn *insn)
{
	u64 ret;
	...
	if (!ghcb_reg_is_valid(ghcb, VMSA_REG_RAX) ||
		...
		return vmg_issue_unsupported(ghcb, SVM_EXIT_RDTSC, 0);
...
static u64 vmg_rdpmc(struct ghcb *ghcb, unsigned long ghcb_pa,
		     struct pt_regs *regs, struct insn *insn)
{
	u64 ret;
	...
	if (!ghcb_reg_is_valid(ghcb, VMSA_REG_RAX) ||
	    !ghcb_reg_is_valid(ghcb, VMSA_REG_RDX))
		return vmg_issue_unsupported(ghcb, SVM_EXIT_RDPMC, 0);
		     ...
		     struct pt_regs *regs, struct insn *insn)
{
	...
	if (!ghcb_reg_is_valid(ghcb, VMSA_REG_RAX) ||
	    ...
	    !ghcb_reg_is_valid(ghcb, VMSA_REG_RDX))
		return vmg_issue_unsupported(ghcb, SVM_EXIT_CPUID, 0);
...
static u64 vmg_invd(struct ghcb *ghcb, unsigned long ghcb_pa,
		    struct pt_regs *regs, struct insn *insn)
{
...
static u64 vmg_ioio(struct ghcb *ghcb, unsigned long ghcb_pa,
		    struct pt_regs *regs, struct insn *insn)
{
	...
	if (!exit_info_1) {
		...
		return vmg_issue_unsupported(ghcb, SVM_EXIT_IOIO, 0);
	}
...
static u64 vmg_msr(struct ghcb *ghcb, unsigned long ghcb_pa,
		   struct pt_regs *regs, struct insn *insn)
{
	u64 exit_info_1 = 0;
	u64 ret;
	...
	case 0x32:	/* RDMSR */
		...
		break;
	default:
		return vmg_issue_unsupported(ghcb, SVM_EXIT_MSR,
					     insn->opcode.bytes[0]);
	}
	...
	if (!exit_info_1) {
		if (!ghcb_reg_is_valid(ghcb, VMSA_REG_RAX) ||
		    !ghcb_reg_is_valid(ghcb, VMSA_REG_RDX))
			return vmg_issue_unsupported(ghcb, SVM_EXIT_MSR, 1);
...
static u64 vmg_vmmcall(struct ghcb *ghcb, unsigned long ghcb_pa,
		       struct pt_regs *regs, struct insn *insn)
{
	u64 ret;
	...
	if (!ghcb_reg_is_valid(ghcb, VMSA_REG_RAX))
		return vmg_issue_unsupported(ghcb, SVM_EXIT_VMMCALL, 0);
...
static u64 vmg_monitor(struct ghcb *ghcb, unsigned long ghcb_pa,
		       struct pt_regs *regs, struct insn *insn)
{
...
static u64 vmg_mwait(struct ghcb *ghcb, unsigned long ghcb_pa,
		     struct pt_regs *regs, struct insn *insn)
{
	ghcb->save.rax = regs->ax;
	ghcb_reg_set_valid(ghcb, VMSA_REG_RAX);
